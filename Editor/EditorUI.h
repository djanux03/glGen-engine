...
// We need a small struct to pass Scene/Selection state cleanly to the Gizmo
// drawer (Alternatively, pass these fields individually, but a struct is
// cleaner)
struct EditorSelectionState {
  uint32_t &selectedEntityId;              // ECS entity ID (0 = Sun/none)
  std::vector<uint32_t> &selectedEntities; // Multi-select ECS entity IDs
  uint32_t &lastClickedEntity;             // Last clicked entity ID
...
public:
  ...
  EditorUIOutput draw(bool &uiMode, float &walkStep, float &runMult,
                      float &jumpStrength, float &gravity, bool &freezePhysics,
                      float &mouseSensitivity, float &fov, SunFX &sun,
                      FireFX &fire, int &terrainSize, float &terrainSpacing,
                      glm::vec3 &treePos, glm::vec3 &treeScale

  );

  // NEW: Gizmo & Outliner drawer (moved from App.cpp)
  void drawGizmo(bool uiMode, const glm::mat4 &view,
