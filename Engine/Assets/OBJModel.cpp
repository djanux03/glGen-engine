...
{
    if (!m.diffuse_texname.empty()) return m.diffuse_texname;
    if (!m.specular_texname.empty()) return m.specular_texname;
    if (!m.normal_texname.empty()) return m.normal_texname;
    if (!m.bump_texname.empty()) return m.bump_texname;
...
bool OBJModel::loadFromFile(const std::string& objPath)
{
    ...
    if (!err.empty())  std::cout << "OBJ err: " << err << "\n";
    if (!ok) return false;

    // Pre-load material GPU data once
    struct MatGPU { std::string name; glm::vec3 kd; GLuint tex; };
    ...
    if (!materials.empty())
    {
        ...
        for (size_t i = 0; i < materials.size(); ++i)
        {
            ...
            matGpu[i].kd = glm::vec3((float)m.diffuse[0], (float)m.diffuse[1], (float)m.diffuse[2]);
            matGpu[i].tex = 0;

            std::string texName = pickSingleTextureName(m);
            if (!texName.empty())
            {
                std::string texPath = isAbsolutePath(texName) ? texName : joinPath(baseDir, texName);
    ...
    auto expandBounds = [](glm::vec3& mn, glm::vec3& mx, bool& has, const glm::vec3& p)
    {
        mn = glm::min(mn, p);
        mx = glm::max(mx, p);
        has = true;
    ...
    for (const auto& shape : shapes)
    {
        ...
        const std::string objName = shape.name.empty() ? "DefaultObject" : shape.name;
            ...
            if (fv == 3)
            {
                ...
                if (!hasAllNormals)
                {
                    ...
                    if (!glm::any(glm::isnan(n)))
                        ...
                        face[0].normal = face[1].normal = face[2].normal = glm::vec3(0, 1, 0);
                }
...
void OBJModel::setObjectYawDegPivot(const std::string& objectName, float yawDeg, const glm::vec3& pivotLocal)
{
    auto& o = mYawOverride[objectName];
    o.yawDeg = yawDeg;
...
void OBJModel::clearObjectOverrides()
{
    mYawOverride.clear();
}

bool OBJModel::getObjectCenterLocal(const std::string& objectName, glm::vec3& outCenter) const
{
    auto it = mObjectBounds.find(objectName);
    if (it == mObjectBounds.end()) return false;
...
std::vector<std::string> OBJModel::objectNames() const
{
    ...
    for (const auto& kv : mObjectBounds) out.push_back(kv.first);
                                ...
                                glm::vec3& outScale) const
{
    ...
    if (it == mObjectTRS.end()) return false;
...
void OBJModel::clearAllObjectLocalTRS()
{
    mObjectTRS.clear();
}
...
bool OBJModel::getSubmeshCenterLocal(const std::string& materialName, glm::vec3& outCenter) const
{
...
void OBJModel::shutdown()
{
    for (auto& sm : mSubmeshes)
    {
        ...
        // IMPORTANT: textures are shared between multiple submeshes now (same material)
        // so DO NOT delete sm.tex here (or you’ll double-delete).
        sm = {};
    }
    mSubmeshes.clear();
    mObjectBounds.clear();
...
if (hasPivot)
{
    extra =
        ...
        glm::rotate(glm::mat4(1.0f), glm::radians(itYaw->second.yawDeg), glm::vec3(0,1,0)) *
        glm::translate(glm::mat4(1.0f), -pivot);
}
    ...
    if (it != mObjectTRS.end() && it->second.enabled)
    {
        ...
        glm::mat4 localTRS = buildTRS(it->second.posLocal,
                                     it->second.rotDegLocal,
...
void OBJModel::drawDepth(Shader& shadowShader,
                         const glm::vec3& position,
                         const glm::vec3& rotDeg,
                         const glm::vec3& scale)
{
    // IMPORTANT: uniforms require the program to be active; ensure the caller activated it,
    // or uncomment the next line if your Shader class has activate(). [web:2157]
    ...
    for (auto& sm : mSubmeshes)
    {
        if (sm.vertexCount == 0 || sm.vao == 0) continue;
...
void OBJModel::draw(Shader& shader,
                    ...
                    const glm::vec3& scale)
{
    // Same note: uniforms require active program. [web:2157]
    // shader.activate();
