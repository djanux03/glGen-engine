...
void FBXModel::processMesh(const tinygltf::Mesh &mesh) {
  ...
  for (size_t i = 0; i < mesh.primitives.size(); i++) {
    ...
    const tinygltf::Accessor &posAccessor =
        mModel.accessors[primitive.attributes.find("POSITION")->second];
    ...
    for (size_t v = 0; v < posAccessor.count; v++) {
      FBXVertex vertex;
      vertex.pos = glm::vec3(positions[v * 3 + 0], positions[v * 3 + 1],
                             positions[v * 3 + 2]);
      ...
      if (normals) {
        vertex.normal = glm::vec3(normals[v * 3 + 0], normals[v * 3 + 1],
                                  normals[v * 3 + 2]);
      } else {
    ...
    const tinygltf::Accessor &indexAccessor =
        mModel.accessors[primitive.indices];
