...
public:
    ...
    void drawDepth(Shader& shadowShader,
                   const glm::vec3& position,
                   const glm::vec3& rotDeg,
                   const glm::vec3& scale);

    // Existing anchor query (material-based):
    bool getSubmeshCenterLocal(const std::string& materialName, glm::vec3& outCenter) const;

    // NEW: object-based query (o <name> in OBJ):
    bool getObjectCenterLocal(const std::string& objectName, glm::vec3& outCenter) const;

    // NEW: rotate an OBJ object (e.g. "Turret_02") around its local pivot (AABB center).
    void setObjectYawDeg(const std::string& objectName, float yawDeg);
    void clearObjectOverrides();
    void setObjectYawDegPivot(const std::string& objectName, float yawDeg, const glm::vec3& pivotLocal);
    bool getObjectLocalTRS(const std::string& objectName,
                       glm::vec3& outPos,
                       glm::vec3& outRotDeg,
                       glm::vec3& outScale) const;

void setObjectLocalTRS(const std::string& objectName,
                       const glm::vec3& pos,
                       const glm::vec3& rotDeg,
                       const glm::vec3& scale);
